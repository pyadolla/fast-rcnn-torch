...
function GeneralUtils:visualize_attributes(im,boxes,scores,thresh,cl_names,attr)
  local ok = pcall(require,'qt')
  if not ok then
    error('You need to run visualize_detections using qlua')
  end
  ...
  -- print (max_score,idx)
  -- print ("thresh = ",idx_thresh,"max = ",max_score[0])
  ...
  -- local num_boxes = boxes_thresh:size(1)
  local boxes_thresh = boxes
  local num_boxes = 1

  local widths = {}
  local heights = {}  

  widths[1] = boxes_thresh[1][3] - boxes_thresh[1][1]
  heights[1] = boxes_thresh[1][4] - boxes_thresh[1][2]
